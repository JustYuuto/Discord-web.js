import { Channel, ChannelType, guildChannels } from '../../helpers/api';
...
export default class ChannelsList extends Component {
  ...
  connectedCallback() {
    ...
    guildChannels(guildId).then(channels => {
      ...
      channels.filter(c => c.type === ChannelType.GUILD_CATEGORY).forEach(channel => {
        html += `<div class="${css({
        ...
        html += `<div class="${css({
          ...
          userSelect: 'none'
        })} ${channelLinkAndCatCss}" aria-label="${channel.name}">`;
        html += `<div class="${css({
        ...
        html += '<div>';
        const list: Channel[] = [];
        channels.filter(c => c.type !== ChannelType.GUILD_CATEGORY && c.parent_id === channel.id).forEach(channel => {
          list[channel.position] = channel;
        });
        list.filter(c => c.type !== ChannelType.GUILD_CATEGORY && c.parent_id === channel.id).forEach(channel => {
          html += `<a href="/channels/${channel.guild_id}/${channel.id}" class="${css({ '> *': {
            margin: '6px', display: 'flex', alignItems: 'center'
          } })} ${channelLinkAndCatCss}">`;
          if (channel.type === ChannelType.GUILD_ANNOUNCEMENT) {
          ...
          } else if (channel.type === ChannelType.GUILD_TEXT) {
